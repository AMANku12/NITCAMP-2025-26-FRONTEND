        ...
        <AnimatePresence>
          {menuOpen && (
            <motion.div
              ...
              className="md:hidden bg-blue-900/95 backdrop-blur-md border-t border-blue-800 px-2 py-2 space-y-1"
            >
              {isLoggedIn && user && (
                <div className="flex items-center gap-3 px-2 py-2 border-b border-blue-800 mb-2">
                  <img
                    src={user.photo_url}
                    alt={user.fullname}
              ...
              {isLoggedIn && ["mentor", "mentee"].includes(user.role) && (
                ...
                >
                  <button
                    ...
                    className="relative block px-2 py-2 text-sm font-medium text-blue-100 hover:text-white transition-colors duration-200 group"
                  >
                    Dashboard
                    <div className="absolute bottom-1 left-1/2 w-0 h-0.5 bg-white group-hover:w-3/4 group-hover:left-2 transition-all duration-300 ease-out" />
                  </button>
                </motion.div>
              )}
              {!isLoggedIn && filteredNav.map((item, i) => (
                <motion.div key={item.path} variants={ANIMATION_VARIANTS.menuItem} transition={{ delay: i * 0.05 }}>
                  <Link
                    to={item.path}
                    onClick={() => setMenuOpen(false)}
                    className={`relative flex items-center gap-2 px-2 py-2 text-sm font-medium transition-colors duration-200 group ${
                      location.pathname === item.path
                        ? "text-white"
                        : "text-blue-100 hover:text-white"
                    }`}
              ...
              {isLoggedIn && user && user.role === "newuser" && (
                <motion.div variants={ANIMATION_VARIANTS.menuItem} transition={{ delay: (["mentor", "mentee"].includes(user.role) ? 1 : 0) * 0.05 }}>
                  <Link
                    to="/choice"
                    onClick={() => setMenuOpen(false)}
              ...
              {isLoggedIn && ["mentor", "mentee"].includes(user.role) && (
                <motion.div variants={ANIMATION_VARIANTS.menuItem} transition={{ delay: 1 * 0.05 }}>
                  <Link
                    to={user.role === "mentor" ? "/mentorprofile" : "/menteeprofile"}
                    onClick={() => setMenuOpen(false)}
                    className="relative block px-2 py-2 text-sm font-medium text-blue-100 hover:text-white transition-colors duration-200 group"
              ...
              {isLoggedIn && (
                <motion.div variants={ANIMATION_VARIANTS.menuItem} transition={{ delay: (["mentor", "mentee"].includes(user.role) ? 2 : 1) * 0.05 }}>
                  <button
                    onClick={handleLogout}
                    className="relative w-full text-left px-2 py-2 text-sm font-medium text-red-400 hover:text-red-300 transition-colors duration-200 group"
                  >
                    Logout
                    <div className="absolute bottom-1 left-1/2 w-0 h-0.5 bg-red-400 group-hover:w-3/4 group-hover:left-2 transition-all duration-300 ease-out" />
                  </button>
                </motion.div>
              )}
            </motion.div>
