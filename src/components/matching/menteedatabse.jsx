  ...
  if (!mentees.length) {
    return (
      <div className="text-white p-2 sm:p-4">
        ...
        <div className="text-center text-gray-300 py-6 sm:py-10 text-xs sm:text-base">No mentees found.</div>
      </div>
    );
  }
        ...
        {mentees.map((mentee, idx) => (
          <div key={mentee.id || idx} className="bg-white text-black rounded-xl p-2 sm:p-4">
            <div className="flex justify-between items-start mb-2 sm:mb-3">
              <div>
                <span className="font-bold text-xs sm:text-lg">#{idx + 1}</span>
                <h3 className="font-bold text-xs sm:text-lg">{mentee.user?.fullname || "-"}</h3>
              </div>
            </div>
          ...
          <table className="w-full text-black bg-white rounded-xl overflow-hidden text-xs sm:text-sm">
            <thead className="bg-gray-200 text-xs">
              <tr>
                ...
                <th className="px-2 sm:px-3 py-2 sm:py-3 text-left font-medium min-w-[80px] sm:min-w-[100px]">Branch Code</th>
                <th className="px-2 sm:px-3 py-2 sm:py-3 text-left font-medium min-w-[80px] sm:min-w-[100px]">Degree Code</th>
                <th className="px-2 sm:px-3 py-2 sm:py-3 text-left font-medium min-w-[90px] sm:min-w-[120px]">Year of Admission</th>
                <th className="px-2 sm:px-3 py-2 sm:py-3 text-left font-medium min-w-[120px] sm:min-w-[180px]">Broad Area</th>
                <th className="px-2 sm:px-3 py-2 sm:py-3 text-left font-medium min-w-[120px] sm:min-w-[180px]">Narrow Area</th>
              </tr>
            </thead>
            <tbody>
              {mentees.map((mentee, idx) => (
                <tr key={mentee.id || idx} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="px-2 sm:px-3 py-2 sm:py-3">{idx + 1}</td>
                  <td className="px-2 sm:px-3 py-2 sm:py-3 font-medium">{mentee.user?.fullname || "-"}</td>
                  <td className="px-2 sm:px-3 py-2 sm:py-3 break-words">{mentee.user?.email || "-"}</td>
                  <td className="px-2 sm:px-3 py-2 sm:py-3">{mentee.branch_code || "-"}</td>
                  <td className="px-2 sm:px-3 py-2 sm:py-3">{mentee.degree_code || "-"}</td>
                  <td className="px-2 sm:px-3 py-2 sm:py-3">{mentee.year_of_admission || "-"}</td>
